...
// @match        http://*/*
// @match        https://*/*
// @icon         https://github.com/FallenMax/smart-toc/raw/master/src/background/icon.png
// @homepage     https://github.com/FallenMax/smart-toc
// @grant        none
// @updateURL    https://raw.githubusercontent.com/twio142/userscripts/refs/heads/main/smartTOC.user.js
// @downloadURL  https://raw.githubusercontent.com/twio142/userscripts/refs/heads/main/smartTOC.user.js
// ==/UserScript==
...
(function () {
  ...
  const smoothScroll = ({ target, scroller, topMargin = 0, maxDuration = 300, callback, prevTest = false }) => {
    ...
    if (prevTest && Math.abs(distance) < 1)
      return 'curAtPrev';
  ...
  let hyperscriptVnode = function () {
    let attrs = arguments[this]; let start = this + 1; let children;
  ...
  function compileSelector(selector) {
    let match; let tag = 'div'; let classes = []; let attrs = {};
    while (match = selectorParser.exec(selector)) {
      let type = match[1]; let value = match[2];
      if (type === '' && value !== '') {
        tag = value;
      } else if (type === '#') {
        attrs.id = value;
  ...
  function execSelector(state, vnode$1) {
    let attrs = vnode$1.attrs;
    let children = vnode.normalizeChildren(vnode$1.children);
  ...
  function hyperscript(selector) {
    ...
    let vnode$1 = hyperscriptVnode.apply(1, arguments);

    if (typeof selector === 'string') {
      vnode$1.children = vnode.normalizeChildren(vnode$1.children);
      if (selector !== '[')
        return execSelector(selectorCache[selector] || compileSelector(selector), vnode$1);
    }
  ...
  let hyperscript_1 = hyperscript;

  let trust = function (html) {
    if (html == null)
      html = '';
  ...
  let fragment = function () {
    let vnode$1 = hyperscriptVnode.apply(0, arguments);
  ...
  let hyperscript_1$1 = hyperscript_1;
  ...
  let PromisePolyfill = function (executor) {
    ...
    let self = this; let resolvers = []; let rejectors = []; let resolveCurrent = handler(resolvers, true); let rejectCurrent = handler(rejectors, false);
    let instance = self._instance = { resolvers, rejectors };
  ...
  PromisePolyfill.prototype.then = function (onFulfilled, onRejection) {
    let self = this; let instance = self._instance;
  ...
  PromisePolyfill.all = function (list) {
    return new PromisePolyfill((resolve, reject) => {
      let total = list.length; let count = 0; let values = [];
      if (list.length === 0) {
        resolve([]);
      } else {
  ...
  let polyfill = PromisePolyfill;

  let promise = createCommonjsModule((module) => {
  ...
  let render = function ($window) {
    ...
    function createHTML(parent, vnode, ns, nextSibling) {
      let match = vnode.children.match(/^\s*<(\w+)/m) || [];
    ...
    function createElement(parent, vnode$1, hooks, ns, nextSibling) {
      ...
      if (!maybeSetContentEditable(vnode$1)) {
        if (vnode$1.text != null) {
          if (vnode$1.text !== '')
            element.textContent = vnode$1.text;
        ...
        if (vnode$1.children != null) {
          ...
          if (vnode$1.tag === 'select' && attrs != null)
            setLateSelectAttrs(vnode$1, attrs);
        }
      }
    }
    ...
    function updateNodes(parent, old, vnodes, hooks, nextSibling, ns) {
      if (old === vnodes || old == null && vnodes == null) {
        ...
        if (isKeyed === null && isOldKeyed == null)
          return; // both lists are full of nulls
        if (isOldKeyed !== isKeyed) {
          ...
          // keyed diff
          let oldEnd = old.length - 1; let end = vnodes.length - 1; var map; var o; var v; let oe; let ve; let topSibling;
          ...
          if (start > end) {
            ...
            let originalNextSibling = nextSibling; let vnodesLength = end - start + 1; let oldIndices = new Array(vnodesLength); let li = 0; let i = 0; let pos = 2147483647; let matched = 0; var map; let lisIndices;
    ...
    function updateNode(parent, old, vnode, hooks, nextSibling, ns) {
      let oldTag = old.tag; let tag = vnode.tag;
      if (oldTag === tag) {
    ...
    function updateFragment(parent, old, vnode, hooks, nextSibling, ns) {
      ...
      let domSize = 0; let children = vnode.children;
      vnode.dom = null;
      if (children != null) {
    ...
    function updateElement(old, vnode$1, hooks, ns) {
      let element = vnode$1.dom = old.dom;
      ns = getNameSpace(vnode$1) || ns;
      ...
      if (!maybeSetContentEditable(vnode$1)) {
        if (old.text != null && vnode$1.text != null && vnode$1.text !== '') {
          if (old.text.toString() !== vnode$1.text.toString())
            old.dom.firstChild.nodeValue = vnode$1.text;
        } else {
          if (old.text != null)
            old.children = [vnode('#', undefined, undefined, old.text, undefined, old.dom.firstChild)];
          if (vnode$1.text != null)
            vnode$1.children = [vnode('#', undefined, undefined, vnode$1.text, undefined, undefined)];
    ...
    function hasPropertyKey(vnode, key, ns) {
      ...
      // If it's a custom element, just keep it.
        vnode.tag.includes('-') || vnode.attrs != null && vnode.attrs.is
  ...
  let render$1 = render(window);

  let mountRedraw = function (render, schedule, console) {
    let subscriptions = [];
    let rendering = false;
    let pending = false;
  ...
  let mountRedraw$1 = mountRedraw(render$1, requestAnimationFrame, console);

  let build = function (object) {
  ...
  let build$1 = function (template, params) {
    if ((/:([^/.-]+)(\.{3})?:/).test(template)) {
    ...
    let resolved = path.replace(/:([^/.-]+)(\.{3})?/g, (m, key, variadic) => {
      delete query[key];
  ...
  let request = function ($window, Promise, oncompletion) {
    let callbackCount = 0;
    ...
    function makeRequest(factory) {
      return function (url, args) {
        ...
        let promise = new Promise((resolve, reject) => {
          factory(build$1(url, args.params), args, (data) => {
    ...
    return {
      request: makeRequest((url, args, resolve, reject) => {
        let method = args.method != null ? args.method.toUpperCase() : 'GET';
        let body = args.body;
        let assumeJSON = (args.serialize == null || args.serialize === JSON.serialize) && !(body instanceof $window.FormData);
        let responseType = args.responseType || (typeof args.extract === 'function' ? '' : 'json');

        let xhr = new $window.XMLHttpRequest(); let aborted = false;
        let original = xhr; let replacedAbort;
        let abort = xhr.abort;
        ...
        xhr.onreadystatechange = function (ev) {
          ...
          if (ev.target.readyState === 4) {
            try {
              ...
              let response = ev.target.response; let message;
      ...
      jsonp: makeRequest((url, args, resolve, reject) => {
        let callbackName = args.callbackName || `_mithril_${Math.round(Math.random() * 1e16)}_${callbackCount++}`;
  ...
  let request$1 = request(window, promise, mountRedraw$1.redraw);
  ...
  let parse = function (string) {
    ...
    let entries = string.split('&'); let counters = {}; let data = {};
    for (let i = 0; i < entries.length; i++) {
      ...
      for (let j = 0; j < levels.length; j++) {
        let level = levels[j]; let nextLevel = levels[j + 1];
        let isNumber = nextLevel == '' || !isNaN(Number.parseInt(nextLevel, 10));
        if (level === '') {
  ...
  let parse$1 = function (url) {
    let queryIndex = url.indexOf('?');
    let hashIndex = url.indexOf('#');
    let queryEnd = hashIndex < 0 ? url.length : hashIndex;
    let pathEnd = queryIndex < 0 ? queryEnd : queryIndex;
  ...
  let compileTemplate = function (template) {
    let templateData = parse$1(template);
    let templateKeys = Object.keys(templateData.params);
    let keys = [];
    let regexp = new RegExp(`^${templateData.path.replace(
    ...
    return function (data) {
      ...
      let values = regexp.exec(data.path);
      if (values == null)
        return false;
  ...
  let router = function ($window, mountRedraw) {
    let fireAsync;
    ...
    let currentResolver = sentinel; let component; let attrs; let currentPath; let lastUpdate;
    ...
    function route(root, defaultRoute, routes) {
      ...
      let compiled = Object.keys(routes).map((route) => {
        if (route[0] !== '/')
      ...
      if (defaultRoute != null) {
        let defaultData = parse$1(defaultRoute);
      ...
      function resolveRoute() {
        ...
        let path = prefix.concat()
          .replace(/(?:%[a-f89][a-f0-9])+/gi, decodeURIComponent)
          .slice(route.prefix.length);
        let data = parse$1(path);
      ...
      return mountRedraw.mount(root, {
        ...
        view() {
          ...
          let vnode$1 = [vnode(component, attrs.key, attrs)];
          if (currentResolver)
            vnode$1 = currentResolver.render(vnode$1[0]);
          return vnode$1;
        },
      });
    }
    ...
    route.Link = {
      view(vnode) {
        ...
        let attrs = {}; let onclick; let href;
        ...
        // should honor that.
        let child = hyperscript_1(vnode.attrs.selector || 'a', attrs, vnode.children);
  ...
  let route = router(window, mountRedraw$1);

  let m = function m() {
    return hyperscript_1$1.apply(this, arguments);
  };
  m.m = hyperscript_1$1;
  ...
  let tocCSS = '#smarttoc {all: initial;}#smarttoc *,#smarttoc *::before,#smarttoc *::after {all: unset;}#smarttoc {display: flex;flex-direction: column;align-items: stretch;position: fixed;left: 0px;top: 0px;min-width: 14em;max-width: 50vw;max-height: 100vh;z-index: 10000;box-sizing: border-box;background-color: #fff;color: var(--gray);font-size: calc(12px + 0.1vw);font-family: \'SF Pro Display\', \'Helvetica Neue\', Helvetica, Arial, sans-serif;line-height: 1.5;font-weight: normal;border: 1px solid rgba(158, 158, 158, 0.22);border-radius: 6px;-webkit-font-smoothing: subpixel-antialiased;font-smoothing: subpixel-antialiased;overflow: hidden;will-change: transform, max-width;transition: max-width 0.3s;contain: content;}#smarttoc.hidden {display: none;}#smarttoc .handle {-webkit-user-select: none;user-select: none;border-bottom: 1px solid rgba(158, 158, 158, 0.22);padding: 0.1em 0.7em;font-variant-caps: inherit;font-variant: small-caps;font-size: 0.9em;color: #bbb;cursor: pointer;text-align: center;opacity: 0;will-change: opacity;transition: opacity 0.3s;}#smarttoc:hover .handle {max-width: 100vw;opacity: 1;}#smarttoc .handle:hover {cursor: grab;}#smarttoc .handle:active {cursor: grabbing;}#smarttoc .handle:active {background: #f9f9f9;}#smarttoc > ul {flex-grow: 1;padding: 0 1.3em 1.3em 1em;overflow-y: auto;} /* all headings  */#smarttoc ul,#smarttoc li {list-style: none;display: block;}#smarttoc a {text-decoration: none;color: var(--gray);display: block;line-height: 1.4;padding-top: 0.2em;padding-bottom: 0.2em;text-overflow: ellipsis;overflow-x: hidden;white-space: nowrap;cursor: pointer;}#smarttoc a:hover,#smarttoc a:active {border-left-color: rgba(var(--purple), 0.5);color: rgb(var(--purple));}#smarttoc li.active > a {border-left-color: rgb(var(--purple));color: rgb(var(--purple));} /* heading level: 1 */#smarttoc ul {line-height: 2;}#smarttoc ul a {font-size: 1em;padding-left: 1.3em;}#smarttoc ul a:hover,#smarttoc ul a:active,#smarttoc ul li.active > a {border-left-width: 3px;border-left-style: solid;padding-left: calc(1.3em - 3px);}#smarttoc ul li.active > a {font-weight: 700;} /* heading level: 2 (hidden only when there are too many headings)  */#smarttoc ul ul {line-height: 1.8;}#smarttoc.lengthy ul ul {display: none;}#smarttoc.lengthy ul li.active > ul {display: block;}#smarttoc ul ul a {font-size: 1em;padding-left: 2.7em;}#smarttoc ul ul a:hover,#smarttoc ul ul a:active,#smarttoc ul ul li.active > a {border-left-width: 2px;border-left-style: solid;padding-left: calc(2.7em - 2px);font-weight: normal;} /* heading level: 3 (hidden unless parent is active) */@keyframes blinking { 0% {background-color: rgba(var(--purple), 0.25);} 100% {background-color: transparent;} } .toc-active {animation: blinking 1.6s ease;} #smarttoc ul ul ul {line-height: 1.7;display: none;}#smarttoc ul ul li.active > ul {display: block;}#smarttoc ul ul ul a {font-size: 1em;padding-left: 4em;}#smarttoc ul ul ul a:hover,#smarttoc ul ul ul a:active,#smarttoc ul ul ul li.active > a {border-left-width: 1px;border-left-style: solid;padding-left: calc(4em - 1px);font-weight: normal;}:root {--purple: 86,61,124;--gray: #808080;}@media (prefers-color-scheme: dark) {:root {--purple: 136,101,189;--gray: #a5a5a5;}#smarttoc {background-color: rgba(30, 30, 30, 0.98);}#smarttoc .handle:active {background: #393939;}}';
      ...
      return new Promise((resolve, reject) => {
        ...
        if (heading) {
          if (smoothScroll({
            ...
            prevTest: headingIndex > 0,
            callback() {
              $triggerTopbarMeasure(heading.dom);
              // resolve();
            },
          }) === 'curAtPrev') {
            heading = headings[headingIndex - 1];
            if (heading) {
              smoothScroll({
                target: heading.dom,
                scroller: scroller.dom,
        ...
        dom.classList.add('toc-active');
        new Promise(resolve => setTimeout(() => resolve(
          dom.classList.remove('toc-active'),
        ), 2e3));
      });
    };
  ...
  if (window === getContentWindow()) {
    ...
    const start = () => {
      ...
      if (!(article && headings && headings.length)) {
        showToast('No article/ headings detected.');
        return;
      }
      if (toc) {
        toc.dispose();
    ...
    document.addEventListener('keydown', (e) => {
      ...
      } else if (e.code === 'KeyK' && !e.ctrlKey && e.altKey && !e.shiftKey && !e.metaKey) {
        toc.prev();
      } else if (e.code === 'KeyJ' && !e.ctrlKey && e.altKey && !e.shiftKey && !e.metaKey) {
        toc.next();
      }
    });
  }
