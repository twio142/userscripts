...
// @version         23.6.2
// @require         https://raw.githubusercontent.com/twio142/userscripts/refs/heads/main/geniusLyrics.user.js
...
// @sandbox         JavaScript
// @updateURL       https://raw.githubusercontent.com/twio142/userscripts/refs/heads/main/spotifyGeniusLyrics.user.js
// @downloadURL     https://raw.githubusercontent.com/twio142/userscripts/refs/heads/main/spotifyGeniusLyrics.user.js
// ==/UserScript==
...
function setFrameDimensions (container, iframe, bar) {
  iframe.style.width = container.clientWidth - 2 + 'px'
  iframe.style.height = document.documentElement.clientHeight - bar.clientHeight - 15 + 'px'
  if (iframe.contentDocument?.querySelector('#application')) {
    iframe.contentDocument.querySelector('#application').style.width = iframe.style.width
    iframe.contentDocument.querySelector('#application > main').style.width = '97%'
  }
}
...
function onResize () {
  ...
  if (iframe) {
    setFrameDimensions(document.getElementById('lyricscontainer'), iframe, document.querySelector('.lyricsnavbar'))
  }
}
function initResize () {
...
function listSongs (hits, container, query) {
  ...
  backToSearchButton.addEventListener('click', function backToSearchButtonClick (ev) {
    ...
    if (query) {
      ...
      showSearchField(genius.current.title + ' ' + genius.current.artists)
...
function observeIframe() {
  ...
  if (iframe) {
    iframe.addEventListener('load', function () {
      const observer = new MutationObserver(function (mutations, observer) {
        for (const mutation of mutations) {
          if (mutation.type === 'childList' && iframe.contentDocument?.querySelector('#application')) {
            setFrameDimensions(document.getElementById('lyricscontainer'), iframe, document.querySelector('.lyricsnavbar'));
            observer.disconnect();
            break;
          }
    ...
    const bodyObserver = new MutationObserver(function (mutations, bodyObserver) {
      for (const mutation of mutations) {
        if (mutation.type === 'childList') {
          const newIframe = document.getElementById('lyricsiframe');
          if (newIframe) {
...
function observePlaying() {
  const nowPlayingFooter = document.querySelector('[data-testid="now-playing-widget"]')
  if (playingObserver || !nowPlayingFooter) {
      console.log('something wrong')
      return
  }
  var prevValue = nowPlayingFooter.getAttribute('aria-label')
  var playingObserver = new MutationObserver((mutationsList) => {
      for (let mutation of mutationsList) {
          if (mutation.type === 'attributes' && mutation.attributeName === 'aria-label') {
              const currentValue = nowPlayingFooter.getAttribute('aria-label')
  ...
  playingObserver.observe(nowPlayingFooter, { attributes: true })
}
...
function showSearchField (query) {
  ...
  if (query) {
    ...
    input.value = genius.current.title + ' ' + genius.current.artists
  } else if (genius.current.artists) {
